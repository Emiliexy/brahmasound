...
const articles = [
  ...
  },
  {
    ...
    excerpt: '在佛教文化中，观世音菩萨是大慈大悲的象征，她普渡众生、化解苦难，是许多信徒心中的庇护神。她被广泛崇敬，尤其在中国...',
    date: '2025-02-12',
...
const PracticeSection = () => {
  return (
    <div className="max-w-7xl mx-auto bg-gradient-to-br from-bg-cream/95 via-light-gold/30 to-bg-cream/95 backdrop-blur-sm rounded-2xl shadow-lg p-8 lg:p-12 border border-primary-gold/10">
      {/* 标题区域 */}
      <div className="flex items-center gap-3 mb-12">
        <BookOpenIcon className="w-7 h-7 text-primary-gold" />
        <h2 className="text-2xl lg:text-3xl font-kai font-bold text-burgundy">
          佛法修行
        </h2>
      </div>
        ...
        {articles.map((article, index) => (
          ...
          >
            ...
            <div className="p-6 flex flex-col flex-grow">
              <h3 className="text-lg lg:text-xl font-song font-bold text-dark-brown mb-3 line-clamp-2 hover:text-burgundy transition-colors">
                {article.title}
              </h3>
              <p className="text-gray-600 text-sm mb-4 line-clamp-3 flex-grow">
                {article.excerpt}
              </p>
              <div className="flex justify-between items-center mt-auto pt-4 border-t border-primary-gold/10">
                <time className="text-sm text-gray-500">
                  {article.date}
                </time>
                <Link
                  href={article.link || '#'}
                  className="inline-flex items-center px-4 py-1.5 bg-burgundy/90 text-white rounded-full hover:bg-burgundy transition-colors text-sm font-kai"
                >
                  阅读全文
                </Link>
              </div>
